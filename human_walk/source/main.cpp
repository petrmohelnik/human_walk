...
int main(int argc, char **argv)
{
	...
	std::shared_ptr<SkeletonRenderer> skeletonRenderer(new SkeletonRenderer);
	skeletonRenderer->init(glm::vec3(0.0, -1.0f, 0.0), skeleton);
	if (!skeletonRenderer->initRenderer(m2, s.getProgram("basic_program"))) {
		cin.get();
		return -1;
	}
